...
class InodeNumberLayer():
	...
	def new_inode_number(self, type, parent_inode_number, name):
		...
		for i in range(0, config.MAX_NUM_INODES):
			if self.INODE_NUMBER_TO_INODE(i) == False:                                      #FALSE INDICTES UNOCCUPIED INODE ENTRY HENCE, FREE NUMBER
	...
	def write(self, inode_number, offset, data, parent_inode_number):
                parent_inode = self.INODE_NUMBER_TO_INODE(parent_inode_number)                          # Get parent inode
                child_inode = self.INODE_NUMBER_TO_INODE(inode_number)                                  # Get child inode
                if parent_inode.type != 1:
                    print("Error- Not a directory")
                    return -1
                elif child_inode.type != 0:
                    print("Error- Not a file")
                    return -1
                else:
                        write = interface.write(child_inode, offset, data)                              # call inodelayer write section
                        self.update_inode_table(child_inode, inode_number)
                
                        
	#IMPLEMENTS READ FUNCTIONALITY
	def read(self, inode_number, offset, length, parent_inode_number):
                parent_inode = self.INODE_NUMBER_TO_INODE(parent_inode_number)
                child_inode = self.INODE_NUMBER_TO_INODE(inode_number)
                if not child_inode:
                        print("Invalid Inode number")
                elif child_inode.type != 0:
                        print("Inode number is not allocated to a file")
                elif not parent_inode:
                        print("Invalid Inode number")
                elif parent_inode.type != 1:
                        print("Inode number is not allocated to a directory")
                read = interface.read(child_inode, offset, length)
                self.update_inode_table(read[0], inode_number)
                return_val = ''.join(read[1])
                return return_val
