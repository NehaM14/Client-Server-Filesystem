...
'''  
import config, DiskLayout,pickle
...
class Operations():
	...
	def get_data_block(self, block_number):
		...
		if block_number == 0: return_value.append("Memory: Reserved for Boot Block")
		elif block_number == 1: return_value.append("Memory: Reserved for Super Block")
		elif block_number >= sblock.BITMAP_BLOCKS_OFFSET and block_number < sblock.INODE_BLOCKS_OFFSET:
			return_value.append( sblock.ADDR_BITMAP_BLOCKS[block_number - sblock.BITMAP_BLOCKS_OFFSET].block)
			return pickle.dumps(return_value)
		elif block_number >= sblock.INODE_BLOCKS_OFFSET and block_number < sblock.DATA_BLOCKS_OFFSET:
			return_value.append(sblock.ADDR_INODE_BLOCKS[block_number - sblock.INODE_BLOCKS_OFFSET].block)
			return pickle.dumps(return_value)
		elif block_number >= sblock.DATA_BLOCKS_OFFSET and block_number < sblock.TOTAL_NO_OF_BLOCKS:
			return_value.append(sblock.ADDR_DATA_BLOCKS[block_number - sblock.DATA_BLOCKS_OFFSET].block)
			return pickle.dumps(return_value)
		else: return_value.append("Memory: Block index out of range or Wrong input!")
	...
	def get_valid_data_block(self):	
		...
		for i in range(0, sblock.TOTAL_NO_OF_BLOCKS):
			if sblock.ADDR_BITMAP_BLOCKS[i / sblock.BLOCK_SIZE].block[i % sblock.BLOCK_SIZE] == 0:
				...
				return pickle.dumps(return_value)
		msg="Memory: No valid blocks available"
	...
	def inode_number_to_inode(self, inode_number):
		inode_number=pickle.loads(inode_number)
		return_value= sblock.ADDR_INODE_BLOCKS[inode_number / sblock.INODES_PER_BLOCK].block[inode_number % sblock.INODES_PER_BLOCK]
		return pickle.dumps(return_value)
