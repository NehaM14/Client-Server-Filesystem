...
class FileNameLayer():
	...
	def read(self, path, inode_number_cwd, offset, length):
		...
		read = interface.read(inode_number, offset, length, parent_inode_number)
		return read
	...
	def write(self, path, inode_number_cwd, offset, data):
		...
		if parent_inode.type != 1:
                ...
                childname = path.split('/')[-1]
                inode_number = self.CHILD_INODE_NUMBER_FROM_PARENT_INODE_NUMBER(childname, parent_inode_number)
		write = interface.write(inode_number, offset, data, parent_inode_number)
	

	#HARDLINK
	def link(self, old_path, new_path, inode_number_cwd):
		'''WRITE YOUR CODE HERE'''
		old_parent_num = self.LOOKUP(old_path)
		old_parent_inode = interface.INODE_NUMBER_TO_INODE(old_parent_num)
		old_parent_inode.directory['/'.split(old_path)[-1]] = file_inode_number
		hardlink_parent_inode_number = self.LOOKUP(new_path)
		hardlink_name = '/'.split(new_path)[-1]
		link = interface.link(file_inode_number, hardlink_name, hardlink_parent_inode_number)
