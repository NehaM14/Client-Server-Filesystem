...
class InodeLayer():
    ...
    def write(self, inode, offset, data):
        ...
        blk_data = self.INODE_TO_BLOCK(inode, offset)                                                                       #Is this an array or returns single block?
        ...
        if inode.size == 0:                                                         # if file is empty continue
            ...
            bytesToTruncate = 0                                                                                                     #why to define it null
            ...
            num_blks_required = int(math.ceil(float(len(new_data)) / config.BLOCK_SIZE))                                            #What happens if not taken formula
            ...
            blkIndexes = list(range(blk_index, end_blk_index))
                                                                                                                        # What happens if below for loop is not applied
            ...
            if offset < inode.size:                                                 #updating the file size
                inode.size = inode.size + len(data) - bytesToTruncate - (inode.size - offset)                 #Why is defining inode size important if BN are updated?                                  
            else:
